...
class Intermediate extends StatefulWidget {
  ...
  const Intermediate({super.key , required this.username});

  @override
  State<Intermediate> createState() => _BeginnerState();
}

class _BeginnerState extends State<Intermediate> with SingleTickerProviderStateMixin {
  int currentId = 1;
  ...
  void initState() {
    ...
    _audioPlayer.play(AssetSource('audio/intermediate.mp3'));
  ...
  Future<void> _loadTotalQuestionsAndFirst() async {
    final snapshot = await FirebaseFirestore.instance.collection('intermediate').get();
  ...
  Timer.periodic(const Duration(seconds: 1), (timer) {
    if (_countdown > 1) {
      ...
      setState(() {
        ...
        timeLeft = 30; // รีเซ็ตเวลาโจทย์จริงทุกครั้ง
      });
      _startQuestionTimer();
    }
  });
  ...
  void _nextQuestion() {
    ...
    if (currentId == totalQuestions) {
      ...
      if (!isCorrect) {
        _shakeController.forward(from: 0); // ✅ สั่งให้สั่นเมื่อผิด
      }
      ...
      // ถ้าไม่ใช่คำถามสุดท้าย เล่นเสียงตอบถูกหรือผิด
      if (isCorrect) {
        score++;
    ...
    if (!snapshot.exists) {
      ...
      transaction.update(docRef, {
        ...
        _shakeController.forward(from: 0); // ✅ เพิ่มตรงนี้ให้สั่น
      }
